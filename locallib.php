...
function evapares_get_teacherview($cmid, $evapares){
	...
	$table_data_query = "SELECT g.name AS group_name, u.lastname, u.firstname, u.id AS userid,
			   ...
			   INNER JOIN {evapares_evaluations} AS ex ON (ex.alu_evalua_id = ee.alu_evaluado_id AND ex.iterations_id = ei.id)
...
function evapares_get_all_data($cmid, $evaparesname){
	...
	$header = array(
			"N" => "N",
			"Grupo" => "Grupo",
			"Alumno evaluado" => "Alumno evaluado",
			"Email" => "Email",
			"Alumno evaluador" => "Alumno evaluador",
